...
const SUPABASE_URL  = "https://udawpaivdegqhlyvnffs.supabase.cogt";
...
function ErrorBanner({ msg, onRetry }) {
  return (
    <div style={{ margin:16, padding:"14px 16px", background:"#fff3f0", borderRadius:12, border:"1px solid #fcc", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
      <span style={{ fontSize:13, color:"#c04040" }}>{msg}</span>
      {onRetry && <button onClick={onRetry} style={{ fontSize:12, color:"#2d7a58", border:"none", background:"none", cursor:"pointer", fontWeight:600 }}>重试</button>}
    </div>
  );
}
...
function Thumb({ item }) {
  const p = pal(item.id);
  if (item.image_url) {
    return <img src={item.image_url} alt={item.name} style={{ width:68, height:68, borderRadius:12, objectFit:"cover", flexShrink:0 }} />;
  }
  return (
    <div style={{ width:68, height:68, borderRadius:12, flexShrink:0, background:p.bg, display:"flex", alignItems:"center", justifyContent:"center", fontSize:24, fontWeight:700, color:p.fg, fontFamily:"'Noto Serif SC',serif" }}>
      {item.name.charAt(0)}
    </div>
  );
}
  ...
  return (
    <div style={{ display:"flex", gap:10, padding:"12px 0", borderBottom:"1px solid rgba(45,122,88,0.08)", opacity:item.is_available?1:0.5 }}>
      <div style={{ position:"relative", flexShrink:0 }}>
        <Thumb item={item} />
        {!item.is_available && (
          <div style={{ position:"absolute", inset:0, borderRadius:12, background:"rgba(255,255,255,0.6)", display:"flex", alignItems:"center", justifyContent:"center" }}>
            <span style={{ fontSize:10, color:"#888", fontWeight:600 }}>售罄</span>
          </div>
        )}
      </div>
...
function HistoryPage({ orders, loading, error, onRetry }) {
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"16px 16px 32px" }}>
      {orders.length === 0 ? (
        <div style={{ textAlign:"center", padding:"64px 0", color:"#7a9a85" }}>
          <div style={{ fontSize:36, marginBottom:12 }}>◎</div>
          <div style={{ fontSize:14 }}>暂无历史订单</div>
        </div>
      ) : [...orders].reverse().map(order => (
        <div key={order.id} onClick={() => setExpanded(expanded === order.id ? null : order.id)} style={{ background:"rgba(255,255,255,0.8)", backdropFilter:"blur(12px)", borderRadius:16, marginBottom:12, overflow:"hidden", border:"1px solid rgba(255,255,255,0.9)", boxShadow:"0 2px 12px rgba(45,122,88,0.07)", cursor:"pointer" }}>
          <div style={{ padding:"14px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div>
              <div style={{ fontSize:11, color:"#7a9a85", marginBottom:3 }}>{order.order_no}</div>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              <span style={{ fontSize:12, fontWeight:600, color:STATUS_COLOR[order.status], background:STATUS_COLOR[order.status]+"20", padding:"4px 10px", borderRadius:20 }}>{STATUS_LABEL[order.status]}</span>
              <span style={{ color:"#ccc", fontSize:12 }}>{expanded === order.id ? "▲" : "▼"}</span>
            </div>
          </div>
          {expanded === order.id && (
            <div style={{ borderTop:"1px solid #f0f5f2", padding:"10px 16px 14px" }}>
              {order.order_items?.map((oi, i) => (
                <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"4px 0", fontSize:13, color:"#3a5a46" }}>
                  <span>{oi.item_name} × {oi.quantity}</span>
                  <span style={{ fontWeight:600 }}>¥{(oi.item_price * oi.quantity).toFixed(2)}</span>
                </div>
              ))}
...
export default function MenuApp() {
  ...
  useEffect(() => { if (page === "history") loadOrders(); }, [page, loadOrders]);

  // ── 购物车操作 ──
  const addToCart   = id => setCart(c => ({ ...c, [id]: (c[id]||0)+1 }));
  const subFromCart = id => setCart(c => {
  ...
  const totalPrice = cartItems.reduce((s,{id,qty})=>{
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      const { data: orderData, error: orderErr } = await supabase
        .from("orders")
        .insert({ order_no, total_amount: totalPrice, status: "pending" })
        .select()
        .single();
      if (orderErr) throw orderErr;

      // 2. 插入 order_items
      const orderItemRows = cartItems.map(({ id, qty }) => {
        const m = menuItems.find(m => m.id === id);
        return { order_id: orderData.id, menu_item_id: id, item_name: m.name, item_price: m.price, quantity: qty };
      });
  ...
  const handleRightScroll = () => {
    ...
    for (const cat of categories) {
      ...
      if (el && el.offsetTop - 16 <= ct.scrollTop) cur = cat.id;
    }
    setActiveCat(cur);
  };
    ...
    <div style={{ height:"100dvh", display:"flex", flexDirection:"column", background:"linear-gradient(160deg,#e8f5ee 0%,#f0faf4 40%,#e4f2f8 100%)", fontFamily:"'Noto Sans SC','PingFang SC',sans-serif", maxWidth:480, margin:"0 auto", overflow:"hidden", position:"relative" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .fab{transition:transform 0.18s ease,box-shadow 0.18s ease;}
      ...
      <div style={{ padding:"48px 16px 12px", background:"linear-gradient(180deg,rgba(45,122,88,0.1) 0%,transparent 100%)", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <h1 style={{ fontSize:22, fontWeight:700, color:"#1a3a2a", fontFamily:"'Noto Serif SC',serif" }}>
            {page === "menu" ? "今日菜单 ✦" : "历史订单"}
          </h1>
          <div style={{ display:"flex", background:"rgba(255,255,255,0.6)", borderRadius:20, padding:3, gap:2, backdropFilter:"blur(8px)", border:"1px solid rgba(45,122,88,0.15)" }}>
            {[["menu","菜单"],["history","历史"]].map(([p,label]) => (
              <button key={p} onClick={() => setPage(p)} style={{ padding:"6px 14px", borderRadius:17, border:"none", cursor:"pointer", background:page===p?"#2d7a58":"transparent", color:page===p?"#fff":"#5a8a70", fontSize:12, fontWeight:page===p?600:400, transition:"all 0.2s" }}>{label}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      {page === "menu" ? (
        loadingMenu ? <Spinner /> :
        menuError   ? <ErrorBanner msg={menuError} onRetry={loadMenu} /> :
              ...
              return (
                <div key={cat.id} ref={el => catRefs.current[cat.id] = el}>
                  <div style={{ padding:"16px 0 4px", fontSize:13, fontWeight:600, color:"#2d7a58", display:"flex", alignItems:"center", gap:6, position:"sticky", top:0, background:"linear-gradient(180deg,rgba(240,250,244,0.97) 70%,transparent 100%)", backdropFilter:"blur(4px)", zIndex:1 }}>
                    ...
                    <span style={{ fontSize:11, color:"#aaa", fontWeight:400 }}>({items.length})</span>
                  </div>
                  {items.map(item => (
        ...
        <HistoryPage orders={orders} loading={loadingOrders} error={ordersError} onRetry={loadOrders} />
      )}

      {/* FAB */}
      {page === "menu" && !loadingMenu && !menuError && (
        <>
          <button className="fab" onClick={() => setShowCart(true)} style={{ position:"fixed", bottom:28, right:20, width:60, height:60, borderRadius:"50%", background:totalQty>0?"linear-gradient(135deg,#2d7a58,#3a9068)":"rgba(255,255,255,0.85)", border:totalQty>0?"none":"1.5px solid rgba(45,122,88,0.25)", backdropFilter:"blur(12px)", boxShadow:totalQty>0?"0 6px 24px rgba(45,122,88,0.45)":"0 4px 16px rgba(0,0,0,0.1)", cursor:"pointer", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center" }}>
          ...
          {totalQty > 0 && (
            <div style={{ position:"fixed", bottom:34, right:88, background:"#1a3a2a", color:"#fff", borderRadius:20, padding:"6px 14px", fontSize:13, fontWeight:700, boxShadow:"0 4px 16px rgba(0,0,0,0.2)", zIndex:100, pointerEvents:"none", animation:"fadeIn 0.2s ease" }}>¥{totalPrice}</div>
          )}
        </>
      )}
      ...
      {/* Cart Sheet */}
      {showCart && (
        <CartSheet cartItems={cartItems} menuItems={menuItems} onAdd={addToCart} onSub={subFromCart} onClear={() => setCart({})} onOrder={handleOrder} onClose={() => setShowCart(false)} totalPrice={totalPrice} ordering={ordering} />
      )}
